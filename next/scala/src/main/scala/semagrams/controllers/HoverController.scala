...
/**
 * The hover controller keeps track of what entity is currently
 * hovered by the mouse. Commands might use the currently hovered
 * entity for certain things.
 */

/**
 * The state of the HoverController: either hovering over an
 * entity, or nothing is hovered.
 */
...
case class HoverController($state: Var[HoverState]) extends Modifier[SvgElement] {
  ...
  /**
   * This most commonly would be used to style a certain entity based
   * on whether or not it was hovered.
   */
  def switchState[A](ent: Entity, hovered: A, unhovered: A): Signal[A] =
    $state.signal.map(
      state => if state.isHovered(ent) then hovered else unhovered
    )
}
