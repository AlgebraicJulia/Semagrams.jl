...
// import semagrams.{Ob, PartVal, Table, Property, PropMap, Elt}
...
case class PartSet[D:PartData](
  ...
  def setProp(f:Property,kvs:Iterable[(UID,f.Value)]): PartSet[D] = this.copy(
  ...
  def softSetProp(f:Property,kvs:Iterable[(UID,f.Value)]): PartSet[D] = this.copy(
  ...
  def optionalSet(f:Property,i:UID,pred:Option[f.Value] => Boolean,v:f.Value) = this.copy(
  ...
  def conditionalSet(f:Property,i:UID,pred:f.Value => Boolean,v:f.Value) = this.copy(
  ...
  def softMerge[Data2:PartData](kvs:Seq[(UID,Data2)]): PartSet[D] = this.copy(
  ...
  def optionalSet(f:Property,pred:Option[f.Value] => Boolean,kvs:Seq[(UID,f.Value)]) = this.copy(
  ...
  def conditionalSet(f:Property,pred:f.Value => Boolean,kvs:Seq[(UID,f.Value)]): PartSet[D] = this.copy(
...
object PartStore:
  ...
  implicit val rw: ReadWriter[Part] = readwriter[(UID,UID,String)].bimap[Part](
