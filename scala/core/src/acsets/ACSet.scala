...
case class ACSet[D: PartData](
  ...
  def tryProp(f: Property, parts: Iterable[Part]): Map[Part, Option[f.Value]] =
  ...
  def setProps(kvs: Iterable[(Part, PropMap)]): ACSet[D] =
    ...
    kvs.groupBy(_._1.ob).foldLeft(this) { case (acset, (ob, kvs)) =>
  ...
  def setProp(f: Property, kvs: Iterable[(Part, f.Value)]): ACSet[D] =
    def helper(
      ...
      partStore =
        acset.partStore.updated(ob.id, acset.partStore(ob.id).setProp(f, kvs))
    )
    kvs.groupBy(_._1.ob).foldLeft(this) { case (acset, (ob, obkvs)) =>
      helper(acset, f, ob, obkvs.mapKeys(_.id))
    }
  ...
  def softSetProp(f: Property, kvs: Iterable[(Part, f.Value)]): ACSet[D] =
    def helper(
      ...
      partStore = acset.partStore
        .updated(ob.id, acset.partStore(ob.id).softSetProp(f, kvs))
    )

    kvs.groupBy(_._1.ob).foldLeft(this) { case (acset, (ob, kvs)) =>
      helper(acset, f, ob, kvs.map((part, v) => (part.id, v)))
    }
        ...
        case schema: DynamicSchema =>
          (
            this.copy(
              ...
              partStore =
                partStore + (ob.id -> partStore(ob.id).addParts(partData))
            ),
            newParts
          )
  ...
  def collectFibers(m: FiberMap): Map[Ob, Seq[Part]] =
    m.foldLeft(Map[Ob, Seq[Part]]()) { case (coll, (f, parts)) =>
      coll + (f.dom -> (coll.get(f.dom).getOrElse(Seq()) ++ parts))
    }
  ...
  def remParts(parts: Iterable[Part], cascade: Boolean = true): ACSet[D] =
    ...
    this.copy(
      partStore = toRemove.foldLeft(partStore) { case (store, (ob, parts)) =>
  ...
  def remObs(obs: Iterable[Ob], cascade: Boolean = true) = schema match
    ...
    case _ =>
      println(
        s"Warning: attempted to remove objects $obs from static schema $schema"
      )
  ...
  def remHoms(fs: Iterable[Hom[_]], cascade: Boolean = true) = schema match
    ...
    case _ =>
      println(
        s"Warning: attempted to remove homs $fs from static schema $schema"
      )
  ...
  def addElts(elts: Iterable[Elt]) = schema match
    ...
    case _ =>
      println(
        s"Warning: attempted to add schema elements $elts to static schema $schema"
      )
      this
