...
def boxSize(text: Option[String], fontSize: Option[Double]): Complex =
  boxSize(text, fontSize, false)
def boxSize(
    text: Option[String],
...
def boxSize(lines: Seq[String], fontSize: Double): Complex =
  lines
    .map(boxSize(_, fontSize))
    .foldLeft(Complex(0, 0))((aggr, line) =>
      Complex(aggr.x max line.x, aggr.y + line.y)
    )
...
def splitString(s: String): Seq[String] =
  ...
  val groups = wds.length match
    ...
    case n if n % 3 == 1 =>
      Seq(
        wds.take(n / 3),
        wds.drop(n / 3).take(n / 3 + 1),
        wds.takeRight(n / 3)
      )
    case n if n % 3 == 2 =>
      Seq(
        wds.take(n / 3 + 1),
        wds.drop(n / 3 + 1).take(n / 3),
        wds.takeRight(n / 3 + 1)
      )
