...
trait NewEntitySource[Model,D:PartData] {self =>
  ...
  def mapData(f:D => D): NewEntitySource[Model,D] = NewEntitySource(
    id.refresh(),
    (m:Model,kvs:EntitySeq[D]) => self.makeEntities(m,kvs).map{ 
      case k -> (spr,init) => k -> (spr,f(init)) 
    }
  )
  ...
  def addPropsBy(f: (EntityTag,EntitySeq[D],D) => PropMap) = NewEntitySource(
    id.refresh(),
    (m:Model,kvs:EntitySeq[D]) => self.makeEntities(m,kvs).map{
      case tag -> (spr,init) => tag -> (spr,init.setProps(f(tag,kvs,init)))
    }
  )

  def softAddPropsBy(f: (EntityTag,EntitySeq[D],D) => PropMap) = NewEntitySource(
    id.refresh(),
    (m:Model,kvs:EntitySeq[D]) => self.makeEntities(m,kvs).map{
      case tag -> (spr,init) => tag -> (spr,init.softSetProps(f(tag,kvs,init)))
    }
  )
...
object NewEntitySource:
  def apply[Model,D:PartData](id:UID,entityConstructor:(Model,EntitySeq[D]) => EntitySeq[D]) = new NewEntitySource[Model,D]{
    val id = id
    def makeEntities(m: Model, kvs: EntitySeq[D]): EntitySeq[D] =
...
trait ACSetSource[D:PartData] extends NewEntitySource[ACSet[D],D]:
  ...
  def matchData(acset:ACSet[D]): Seq[(Match,D)] = shapes(acset.schema).flatMap(matchData(_,acset))

  def matches(shape:Shape,acset:ACSet[D]): Seq[Match] = matchData(shape,acset).map(_._1)
  ...
  yield tag(shape,m) -> (sprite,init(shape).merge(data))  
  ).reverse
...
object ObSource:
  ...
  def apply[D:PartData](id:UID,sprite:Sprite[D],ob:Ob,init:D): ObSource[D] =
    new ObSource(id,sprite,Map(ob -> (init)))
  def apply[D:PartData](id:UID,sprite:Sprite[D],kvs:(Ob | (Ob,D))*): ObSource[D] =
...
case class HomTag(contextId:UID,hom:PartHom,src:Part,tgt:Option[Part]) extends EdgeTag:
...
case class SpanTag(contextId:UID,span:Span,apex:Part,feet:(Part,Option[Part])) extends EdgeTag:
...
trait EdgeSource[D:PartData] extends ACSetSource[D]:
  ...
  def tagSpans(acset:ACSet[D]): Seq[(TagType,(EntityTag,Option[EntityTag]))] = 
    shapes(acset.schema).flatMap(shape => 
...
object SpanSource:
  def apply[D:PartData](vsrc:UID,span:Span,sprite:Sprite[D]): SpanSource[D] =
    new SpanSource[D](UID("SpanSource") -> (vsrc,vsrc),sprite,Map(span -> PartData()))
  def apply[D:PartData](vsrc:UID,sprite:Sprite[D],span:Span,init:D): SpanSource[D] =
    new SpanSource(UID("SpanSource") -> (vsrc,vsrc) ,sprite,Map(span -> init))
  def apply[D:PartData](vsrc:UID,sprite:Sprite[D],kvs:(Span | (Span,D))*): SpanSource[D] =
...
case class HomSource[D:PartData](
  ...
  def shapes(s: Schema): Seq[Shape] = 
    s.homSeq.collect{case f:PartHom => f}
      .filter(test)
...
object HomSource:
  def apply[D:PartData](vsrc:UID,hom:PartHom,sprite:Sprite[D]): HomSource[D] =
    new HomSource[D](UID("HomSource") -> (vsrc,vsrc),sprite,Map(hom -> PartData()))
  def apply[D:PartData](vsrc:UID,sprite:Sprite[D],hom:PartHom,init:D): HomSource[D] =
    new HomSource(UID("HomSource") -> (vsrc,vsrc) ,sprite,Map(hom -> init))
  def apply[D:PartData](vsrc:UID,sprite:Sprite[D],kvs:(PartHom | (PartHom,D))*): HomSource[D] =
    val cleankvs = kvs.map(_ match
