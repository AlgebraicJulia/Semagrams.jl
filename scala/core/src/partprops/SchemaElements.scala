...
extension [X <: (Ob & Generator)](x: X) def obGenerators = Map(x.id -> x)
...
case class ValType[T: ReadWriter](id: UID)
    extends AbstractValType
    with Generator:
...
extension (f: Hom[_, _] & Generator) def path = Seq(f)

case class FKey(id: UID, dom: Ob, codom: Ob)
