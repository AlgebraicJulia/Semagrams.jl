...
object Binding {
   ...
   */
  def process[Model](evt: Event, r: Action.Resources[Model], bindings: Seq[Binding[Model]]): IO[Unit] =
    bindings
      .collectFirst(((b: Binding[Model]) => b.hook(evt).map(b.action(_, r))).unlift)
      .getOrElse(IO(()))
}
