...
trait Spacer {
  ...
  def spaceBy[X,T](f: X => Option[T])(xs: Seq[X]): Map[X, Double] =
    xs.collect(((x:X) => f(x).map((x,_))).unlift).groupBy(_._2).map(
      (_, g) =>
        g.map(_._1).zipWithIndex.map((x, i) => (x, assignPos(i, g.length))).toMap
...
def assignBends[S: IsSchema](edges: Map[Ob, (Hom, Hom)], spacing: Double)(a: ACSet[S]): ACSet[S] = {
  def ends(i: Part) = {
  ...
  val bends = FixedSpacing(spacing, true).spaceBy(sortedEnds)(parts).map(
    (i, bend) => ends(i) match {
