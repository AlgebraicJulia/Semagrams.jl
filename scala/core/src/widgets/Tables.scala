...
/** The editing state ( :Option[(K,Property)] ) is stored in the
 * table; cells and rows pass messages to modify it. There are three
 *  kinds of messages: 
 *    - CellMsg contains low-level information about what values 
 *      were returned and how (e.g., Enter, Esc, etc.).
 *    - TableMsg contains high-level information about modifications
 *      to values and the editing state.
 * 
...
class PropTable[K<:Matchable](name:String,cols:Seq[Property],keys:Seq[Property]):
  ...
  def laminarElt[Model](
    ...
    val eltSig = rowSig.split(_._1) { (row,rowProps,rpSig) =>
      row.laminarElt(
        rpSig.map(_._2),
        editingVar.signal.map(_.flatMap( (k,f) =>
          if k == row.key then Some(f) else None
        )),
        msgObs
      )
    }
...
object PropTable:
  ...
  def apply[K<:Matchable](name:String,cols:Seq[Property],keys:Seq[Property] = Seq()) = 
    keys match
      case Seq() => 
...
case class PropRow[K<:Matchable](key:K,cols:Seq[Property],prv:Option[K],nxt:Option[K]):

  /* Neighboring cells */
  val nbrs: Seq[(K,Property)] = if cols.isEmpty
    then Seq()
    else prv.map((_,cols.last)).toSeq
      ++ cols.map((key,_))
      ++ nxt.map((_,cols.head))
...
case class PropCell[K<:Matchable,T](key:K, col:Property {type Value = T})(tOpt:Option[T]):
    ...
    td(cls := "dcell",
      child <-- editSig.map( _ match
        ...
        case false => tableCell(col,valSig,
          editObs.contramap( (_:Unit) => ToggleMsg(col))
        )
      )
    )
 ...
 *  double click.
 */  
def tableCell(col:Property,vSig:Signal[Option[col.Value]],toggle:Observer[Unit]) = div(
  cls("dcell-text"),
 ...
 */  
def tableInput(col:Property,cellObs:Observer[CellMsg],init:Option[col.Value] = None) = 
  td(cls("dcell"),
      ...
      modFilters.flatMap(mods => 
        ...
        Seq(
          ...
          onEnter.filter(evtFilter(mods))
            .mapToValue
            .map(
              v => EnterMsg(PropChange(col,init,catchEmpty(v))
            )) --> cellObs,
