...
/**
 * We control the behavior of Semagrams using an asynchronous IO monad
 * from cats-effect.
 *
 * This allows the "state" of Semagrams to simply be a continuation for
 * receiving the next event. This makes things like modal interfaces
 * and multi-keypress commands very easy; rather than explicitly representing
 * the state machine as an enum, we can simply write something like
 *
 * ```scala
 * for {
 *   source <- selectSource()
 ...
 * ```
 *
 * where selectSource and selectTarget might involve arbitrary interaction
 * of the user with Semagrams.
 */
 ...
 * alive throughout the lifetime of the application, but the laminar API doesn't
 * know this, so we have to go through the whole process...
 *
 * This function should almost never be called directly, its main purpose is
 * to be used in `nextEvent`
 */
...
/**
 * This is the workhorse connecting Laminar and cats-effect. Given an event stream
 * this function uses asynchronous IO to return the next event in the stream
 *
...
case class KeyBindings[Model](bindings: Map[String, Action[Model, Unit]]) {
  /**
   * This listens for a keybinding, and then executes the action associated with
   * that keybinding. Crucially, this only runs *once*. If you want to run this
   * in a loop, you should provide that loop yourself.
   */
