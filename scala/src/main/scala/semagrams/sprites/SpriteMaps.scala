...
class SpriteMaps[State](
  ...
  private[this] val $renderedSpritesVar: Var[List[Map[Entity, RenderedSprite]]] = Var(spriteMakers.map(_ => Map()))

  val $renderedSprites: Signal[List[Map[Entity, RenderedSprite]]] = $renderedSpritesVar.signal

  val $propMaps: Signal[(List[PropMaps], PropMaps)] = $state.map(
    state => {
      spriteMakers.foldLeft((List[PropMaps](), Map[Entity, PropMap]()))(
        (tup, spriteMaker) => {
          val propMaps = tup._2
          val newSprites = spriteMaker.extractor(state, propMaps).map(
            { case (ent, propMap) => (ent, spriteMaker.middleware.updateProps(ent, propMap)) }
          ).toMap
  ...
  def updateSprites(
    ...
    renderedSpritesList.zip(propMapsList).zip(spriteMakers).map(
      { case ((renderedSprites, propMaps), spriteMaker) => {
         ...
         val newRenderedSprites = added.map(
           ent => {
             val propMap = propMaps(ent)
             val propMapStream = $propMaps.changes.map(_._2.get(ent)).collect { case Some(x) => x }
  ...
  def attach =
    List(
      ...
      children <-- $renderedSprites.map(_.map(_.values.map(_.root).toList).flatten)
    )
}
